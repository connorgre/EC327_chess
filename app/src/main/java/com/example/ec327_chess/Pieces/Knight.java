...
public class Knight extends Piece {
    ...
    public ArrayList<Coordinate> Moves(Board[][] b) {
        ...
        if(x+2 < 8) {
            if(y+1 < 8 && (b[x+2][y+1].getPiece().getWhite() != white)){openMoves.add(new Coordinate(x + 2, y + 1));}
            if(y-1 >=0 && (b[x+2][y-1].getPiece().getWhite() != white)){openMoves.add(new Coordinate(x + 2, y - 1));}
        }
        if(x-2 >=0){
            if(y+1 < 8 && (b[x-2][y+1].getPiece().getWhite() != white)){openMoves.add(new Coordinate(x + 2, y + 1));}
            if(y-1 >=0 && (b[x-2][y-1].getPiece().getWhite() != white)){openMoves.add(new Coordinate(x + 2, y - 1));}
        ...
        if(y+2 < 8){
            if(x+1 < 8 && (b[x+1][y+2].getPiece().getWhite() != white)){openMoves.add(new Coordinate(x+1,y+2));}
            if(x-1 >=0 && (b[x-1][y+2].getPiece().getWhite() != white)){openMoves.add(new Coordinate(x-1,y+2));}
        }
        if(y-2 >=0){
            if(x+1 < 8 && (b[x+1][y-2].getPiece().getWhite() != white)){openMoves.add(new Coordinate(x+1,y+2));}
            if(x-1 >=0 && (b[x-1][y-2].getPiece().getWhite() != white)){openMoves.add(new Coordinate(x-1,y+2));}
